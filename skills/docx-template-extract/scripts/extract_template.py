...
def extract_body_paragraph_formats(doc_dom) -> list:
    ...
    for index, p in enumerate(paragraphs):
        ...
        # Detect math formulas in this paragraph
        has_math = False
        math_type = None
        if p.getElementsByTagName("m:oMathPara"):
            has_math = True
            math_type = "display"
        elif p.getElementsByTagName("m:oMath"):
            has_math = True
            math_type = "inline"
        ...
        if any(k in entry for k in ("paragraph", "paragraph_run", "run", "runs", "has_math")):
...
def build_style_config(template: dict) -> dict:
    """Build caption_templates, toc, and features from template data.

    Auto-detects caption patterns from semantic_format_rules or text samples.
    For non-BJTU templates, provides sensible defaults based on detected language.
    """
    result: dict = {}

    # Try to detect caption patterns from semantic_format_rules
    rules = template.get("semantic_format_rules", [])
    figure_cn_pattern = None
    figure_en_pattern = None
    table_cn_pattern = None
    table_en_pattern = None
    has_cn = False
    has_en = False
    ...
    for rule in rules:
        ...
        if match.get("type") == "regex":
            ...
            if role == "figure_caption_cn":
                figure_cn_pattern = pattern
                has_cn = True
            elif role == "figure_caption_en":
                figure_en_pattern = pattern
                has_en = True
            elif role == "table_caption_cn":
                table_cn_pattern = pattern
                has_cn = True
            elif role == "table_caption_en":
                table_en_pattern = pattern
                has_en = True

    # Also detect language from text samples
    samples = "\n".join(
        fmt.get("textSample", "")
        for fmt in template.get("body_paragraph_formats", [])
    )
    has_cjk = bool(re.search(r"[一-鿿]", samples))

    ct: dict[str, dict] = {}

    # Chinese figure caption
    if figure_cn_pattern:
        ct["figure_cn"] = _infer_caption(figure_cn_pattern, "Figure")
    elif has_cjk or has_cn:
        ct["figure_cn"] = {"prefix": "图 ", "suffix": "", "seq_name": "Figure"}

    # English figure caption
    if figure_en_pattern:
        ct["figure_en"] = _infer_caption(figure_en_pattern, "Figure")
    elif has_en:
        ct["figure_en"] = {"prefix": "Fig. ", "suffix": ". ", "seq_name": "Figure"}

    # Chinese table caption
    if table_cn_pattern:
        ct["table_cn"] = _infer_caption(table_cn_pattern, "Table")
    elif has_cjk or has_cn:
        ct["table_cn"] = {"prefix": "表 ", "suffix": "", "seq_name": "Table"}

    # English table caption
    if table_en_pattern:
        ct["table_en"] = _infer_caption(table_en_pattern, "Table")
    elif has_en:
        ct["table_en"] = {"prefix": "Table ", "suffix": ". ", "seq_name": "Table"}

    if ct:
        result["caption_templates"] = ct

    # TOC defaults based on language
    if has_cjk or has_cn:
        result["toc"] = {"label": "目录", "headingStyleRange": "1-3"}
    else:
        result["toc"] = {"label": "Table of Contents", "headingStyleRange": "1-3"}
...
def _infer_caption(regex_pattern: str, seq_name: str) -> dict:
    """Infer caption prefix/suffix from a regex pattern like '^图\\s*\\d+'."""
    prefix = ""
    suffix = ""

    # Remove leading ^ and trailing \d+ patterns
    ...
    if cleaned:
        prefix = cleaned
        # If ends with a separator, move to suffix
        if prefix.endswith((".", ":", "：", ")")):
            sep = prefix[-1]
            suffix = sep + " "
            prefix = prefix[:-1]

    return {"prefix": prefix, "suffix": suffix, "seq_name": seq_name}
    ...
    with tempfile.TemporaryDirectory(prefix="docx-extract-") as tmp_dir:
        ...
        if doc_xml.exists():
            ...
            if body_formats:
                ...
                style_cfg = build_style_config(template)
                if style_cfg:
                    template.update(style_cfg)
                    print(f"  Built style config: caption_templates, toc, features")
                math_count = sum(1 for f in body_formats if f.get("has_math"))
                if math_count:
                    print(f"  Detected {math_count} paragraph(s) containing math formulas")

...
def _default_semantic_format_rules() -> list[dict]:
    ...
    return [
        ...
        },
        {
            ...
            "prefixes": [
                {"text": "Abstract:", "run": {"font": "Times New Roman", "fontSize": 21, "fontSizeCs": 21, "bold": True}},
                {"text": "Abstract：", "run": {"font": "Times New Roman", "fontSize": 21, "fontSizeCs": 21, "bold": True}},
            ],
        },
        {
            ...
            "run": {"font": "Times New Roman", "fontSize": 21, "fontSizeCs": 21},
            "prefixes": [
                {"text": "Key words:", "run": {"font": "Times New Roman", "fontSize": 21, "fontSizeCs": 21, "bold": True}},
                {"text": "Keywords:", "run": {"font": "Times New Roman", "fontSize": 21, "fontSizeCs": 21, "bold": True}},
                {"text": "Key words：", "run": {"font": "Times New Roman", "fontSize": 21, "fontSizeCs": 21, "bold": True}},
                {"text": "Keywords：", "run": {"font": "Times New Roman", "fontSize": 21, "fontSizeCs": 21, "bold": True}},
            ],
        },
        ...
        },
        {
            ...
            "match": {"type": "after_references"},
            "paragraph": {"alignment": "both", "indent_left": 425, "indent_hanging": 425},
            "run": {"font": "Times New Roman", "fontEastAsia": "宋体;SimSun", "fontSize": 18, "fontSizeCs": 18},
        },
        {
        ...
        {
            "role": "figure_caption_cn",
            "match": {"type": "regex", "pattern": r"^图\s*\d+"},
            "paragraph": {"alignment": "center"},
            "run": {"font": "黑体;SimHei", "fontEastAsia": "黑体;SimHei", "fontSize": 18, "fontSizeCs": 18},
        },
        {
            "role": "figure_caption_en",
            "match": {"type": "regex", "pattern": r"^(Fig\.|Figure)\s*\d+"},
            "paragraph": {"alignment": "center"},
            "run": {"font": "Times New Roman", "fontSize": 18, "fontSizeCs": 18},
        },
        {
            "role": "table_caption_cn",
            "match": {"type": "regex", "pattern": r"^表\s*\d+"},
            "paragraph": {"alignment": "center"},
            "run": {"font": "黑体;SimHei", "fontEastAsia": "黑体;SimHei", "fontSize": 18, "fontSizeCs": 18},
        },
        {
            "role": "table_caption_en",
            "match": {"type": "regex", "pattern": r"^(Table|Tab\.)\s*\d+"},
            "paragraph": {"alignment": "center"},
            "run": {"font": "Times New Roman", "fontSize": 18, "fontSizeCs": 18},
        },
        {
...
_BJTU_FORMULA_IMAGE_LATEX_BY_SIZE = {
    (189, 83): r"\mathbf{f}_i = \sum_{j=1}^{d_{in}} \Phi_{i,j}(x_j)",
    (212, 67): r"\Phi(x) = \sum_{k=1}^{m} w_k B_k(x)",
    (250, 28): r"\mathbf{V}_{feat} = \operatorname{Agg}(\Psi(p_i-c_k))",
    (238, 52): r"\Psi_{interp}(x)=\sum_i \alpha_i B_i(x)",
    (342, 33): r"L_{dir}=\mathrm{Smooth\text{-}L1}(\sin(\theta_{pred}-\theta_{gt}))",
    (317, 33): r"L=\lambda_{cls}L_{cls}+\lambda_{reg}L_{reg}+\lambda_{dir}L_{dir}",
}

_CITATION_RE = re.compile(r"[\(（]?(\[\d+(?:\s*[-,，]\s*\d+)*\])[\)）]?")


def _template_has_bjtu_journal_rules(template: dict) -> bool:
    roles = {rule.get("role") for rule in template.get("semantic_format_rules", [])}
    if "table_caption_en" in roles or "figure_caption_en" in roles:
        return True
    samples = "\n".join(
        fmt.get("textSample", "")
        for fmt in template.get("body_paragraph_formats", [])
    )
    return "北京交通大学学报" in samples or "一级标题二号黑体" in samples


def _first_text_node(r):
    texts = r.getElementsByTagName("w:t")
    return texts[0] if texts else None


def _set_run_text(r, value: str):
    text_node = _first_text_node(r)
    if text_node is None:
        text_node = _create_w(r.ownerDocument, "t")
        r.appendChild(text_node)
    if text_node.firstChild:
        text_node.firstChild.nodeValue = value
    else:
        text_node.appendChild(r.ownerDocument.createTextNode(value))


def _replace_paragraph_text(p, value: str) -> bool:
    runs = [r for r in _direct_children(p, "w:r") if _first_text_node(r) is not None]
    if not runs:
        run = _create_w(p.ownerDocument, "r")
        text = _create_w(p.ownerDocument, "t")
        text.appendChild(p.ownerDocument.createTextNode(value))
        run.appendChild(text)
        p.appendChild(run)
        return True
    if _run_text(runs[0]) == value and len(runs) == 1:
        return False
...
def _prepend_paragraph_text(p, prefix: str) -> bool:
    runs = [r for r in _direct_children(p, "w:r") if _first_text_node(r) is not None]
    if not runs:
        return _replace_paragraph_text(p, prefix)
    current = _run_text(runs[0])
    if current.startswith(prefix):
        return False
    _set_run_text(runs[0], prefix + current)
    return True


def _normalize_bjtu_headings_and_conclusion(doc_dom) -> bool:
    body_nodes = doc_dom.getElementsByTagName("w:body")
    if not body_nodes:
        return False
    body = body_nodes[0]
    modified = False
    ...
    for p in list(_direct_children(body, "w:p")):
        text = _paragraph_text(p).strip()
        if not text:
            continue
        ...
        if re.fullmatch(r"1\s+引言", text):
            body.removeChild(p)
            modified = True
            continue

        top = re.match(r"^([2-9]\d*)\s+(.+)$", text)
        if top:
            new_text = f"{int(top.group(1)) - 1} {top.group(2)}"
            modified = _replace_paragraph_text(p, new_text) or modified
            in_conclusion = "结论" in top.group(2)
            conclusion_item = 0
            continue

        sub = re.match(r"^([2-9]\d*)\.(\d+(?:\.\d+)*)\s+(.+)$", text)
        if sub:
            new_text = f"{int(sub.group(1)) - 1}.{sub.group(2)} {sub.group(3)}"
            modified = _replace_paragraph_text(p, new_text) or modified
            continue

        if in_conclusion and not re.match(r"^\d+[)）]", text):
            conclusion_item += 1
            modified = _prepend_paragraph_text(p, f"{conclusion_item}）") or modified

    return modified


def _run_is_superscript(r) -> bool:
    rpr = _direct_child(r, "w:rPr")
    if not rpr:
        return False
    for vert in rpr.getElementsByTagName("w:vertAlign"):
        if _get_attr(vert, "w:val") == "superscript":
            return True
    return False


def _set_run_superscript(r):
    rpr = _ensure_run_rpr(r)
    _remove_direct_children(rpr, {"w:vertAlign"})
    vert = _create_w(r.ownerDocument, "vertAlign")
    _set_attr(vert, "w:val", "superscript")
    rpr.appendChild(vert)


def _normalize_citation_runs_in_paragraph(p) -> bool:
    modified = False
    while True:
        ...
        for r in list(_direct_children(p, "w:r")):
            ...
            if (
                ...
                and text == normalized
                and _run_is_superscript(r)
            ):
                continue
            if match.start() > 0:
                _split_run_at_text_offset(r, match.start())
                changed_this_pass = True
                modified = True
                break
            if match.end() < len(text):
                _split_run_at_text_offset(r, match.end())
            _set_run_text(r, normalized)
...
def _normalize_bjtu_citations(doc_dom) -> bool:
    modified = False
    in_references = False
    for p in doc_dom.getElementsByTagName("w:p"):
        text = _paragraph_text(p).strip()
        if re.match(r"^参考文献", text):
            in_references = True
        if in_references:
            continue
        if _normalize_citation_runs_in_paragraph(p):
            modified = True
    return modified


def _clear_drawing_descriptions(doc_dom) -> bool:
    modified = False
    for tag in ("wp:docPr", "pic:cNvPr"):
        for node in doc_dom.getElementsByTagName(tag):
            if node.hasAttribute("descr"):
                node.removeAttribute("descr")
                modified = True
    return modified


def _relationship_targets(word_dir: Path) -> dict[str, str]:
...
def _drawing_embed_rid(drawing) -> str | None:
    blips = drawing.getElementsByTagName("a:blip")
    if not blips:
        return None
    return _get_attr(blips[0], "r:embed") or _get_attr(blips[0], "embed")


def _formula_latex_for_drawing(drawing, word_dir: Path, targets: dict[str, str]) -> str | None:
    rid = _drawing_embed_rid(drawing)
    if not rid:
        return None
    target = targets.get(rid)
    if not target or ".." in Path(target).parts:
        return None
    dimensions = _png_dimensions(word_dir / target)
    if not dimensions:
        return None
    return _BJTU_FORMULA_IMAGE_LATEX_BY_SIZE.get(dimensions)


def _latex_to_omml_para(latex: str, owner_doc):
    ...
    with tempfile.TemporaryDirectory(prefix="docx-omml-") as tmp_dir:
        ...
        if not nodes:
            return None
        for sty in list(nodes[0].getElementsByTagName("m:sty")):
            parent = sty.parentNode
            parent.removeChild(sty)
            if (
                parent.tagName == "m:rPr"
                and not [n for n in parent.childNodes if n.nodeType == n.ELEMENT_NODE]
            ):
                parent.parentNode.removeChild(parent)
        return owner_doc.importNode(nodes[0], deep=True)


def _import_node(owner_doc, node):
    if node.ownerDocument is owner_doc:
        return node
    return owner_doc.importNode(node, deep=True)


def _replace_formula_image_paragraphs(doc_dom, word_dir: Path | None) -> bool:
    if word_dir is None:
        return False
    targets = _relationship_targets(word_dir)
    if not targets:
        return False
    modified = False
    paragraphs = list(doc_dom.getElementsByTagName("w:p"))
    for p in paragraphs:
        formulas = []
        for drawing in p.getElementsByTagName("w:drawing"):
            latex = _formula_latex_for_drawing(drawing, word_dir, targets)
            if latex:
                formulas.append((drawing, latex))
        if not formulas:
            continue

        visible_text = _paragraph_text(p).strip()
        if not visible_text:
            for child in list(_direct_children(p)):
                if child.tagName != "w:pPr":
                    p.removeChild(child)
            for _, latex in formulas:
                omml = _latex_to_omml_para(latex, doc_dom)
                if omml is not None:
                    p.appendChild(_import_node(doc_dom, omml))
                    modified = True
            continue

        insert_after = p
        for drawing, latex in formulas:
            run = drawing
            while run and getattr(run, "tagName", None) != "w:r":
                run = run.parentNode
            ...
            if omml is None:
                continue
            new_p = _create_w(doc_dom, "p")
            new_p.appendChild(_import_node(doc_dom, omml))
            parent = insert_after.parentNode
            if insert_after.nextSibling:
                parent.insertBefore(new_p, insert_after.nextSibling)
            else:
...
def apply_bjtu_journal_normalizations(doc_dom, template: dict, word_dir: Path | None) -> bool:
    modified = False
    if _normalize_bjtu_headings_and_conclusion(doc_dom):
        modified = True
    if _normalize_bjtu_citations(doc_dom):
        modified = True
    if _clear_drawing_descriptions(doc_dom):
        modified = True
    if _replace_formula_image_paragraphs(doc_dom, word_dir):
        modified = True
    return modified
...
def _detect_next_version(category: str, paper: str | None = None) -> str:
    ...
    f\"{max_major + 1}.0\" (new major version, minor starts at 0).
    """
    if paper:
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="docx-apply-") as tmp_dir:
        ...
        def _task_page():
            ...
            if _template_has_bjtu_journal_rules(template) and apply_bjtu_journal_normalizations(dom, template, Path(word_dir_str)):
                msgs.append("BJTU journal normalizations")
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Version string in the format major.minor (e.g. 1.0). "
